...
const currentView = ref<ViewState>("scanner");
const hasStarted = ref(false);
const scanError = ref<string | null>(null);
const currentCoverA = ref<string | null>(null);
const currentCoverB = ref<string | null>(null);
const isBgAActive = ref(true);
// const currentCover = computed(() =>
//   isBgAActive.value ? currentCoverA.value : currentCoverB.value,
// );

// --- Hitster (Scan) State ---
const hitsterSong = ref<import("./types").HitsterResult | null>(null);
const activeHitsterSource = ref<"youtube" | "audio" | null>(null);
...
const isLoading = computed(() => {
  ...
  switch (gameMode.value) {
    case "blindtest":
      return dtLoading.value;
    case "timeline":
      return tlLoading.value;
    case "bullseye":
      return beLoading.value;
    case "memory":
      return memLoading.value;
    default:
      return false;
  }
});
...
onMounted(() => {
  ...
  (window as any).onYouTubeIframeAPIReady = () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    playerRef.value = new (window as any).YT.Player("youtube-player", {
      height: "1",
      width: "1",
      playerVars: {
        playsinline: 1,
      ...
      events: {
        ...
        onError: (event: any) => {
          ...
          if (gameMode.value === "hitster")
            audioError.value = "Erreur YouTube (" + event.data + ")";
        },
      },
    });
  };
...
const playHitsterTrack = (resource: string, type: "youtube" | "audio") => {
  ...
  if (type === "youtube" && playerRef.value && playerRef.value.loadVideoById) {
    ...
    if (playerRef.value && playerRef.value.stopVideo)
      playerRef.value.stopVideo();
    playAudio(resource);
  }
};
...
const updateBackground = (newCover: string) => {
  if (!newCover) return;
...
<template>
  <div class="app-container">
      ...
      <div v-if="!hasStarted" class="start-screen">
        <div class="mode-selection">
          <button class="btn-primary btn-mode" @click="setGameMode('hitster')">
            <Scan :size="24" />
            <div class="mode-info">
              <span class="mode-name">Hitster</span
              ><span class="mode-desc">Scan & Play</span>
            </div>
            <ChevronRight :size="20" style="margin-left: auto; opacity: 0.5" />
          </button>
          <button
            class="btn-primary btn-mode btn-blindtest"
            @click="setGameMode('blindtest')"
          >
            <Music :size="24" />
            <div class="mode-info">
              <span class="mode-name">Blindtest</span
              ><span class="mode-desc">Auto-Mix Aléatoire</span>
            </div>
            <ChevronRight :size="20" style="margin-left: auto; opacity: 0.5" />
          </button>
          <button
            class="btn-primary btn-mode btn-timeline"
            @click="setGameMode('timeline')"
          >
            <Calendar :size="24" />
            <div class="mode-info">
              <span class="mode-name">Timeline</span
              ><span class="mode-desc">Plus ou Moins ?</span>
            </div>
            <ChevronRight :size="20" style="margin-left: auto; opacity: 0.5" />
          </button>
          <button
            class="btn-primary btn-mode btn-bullseye"
            @click="setGameMode('bullseye')"
          >
            <Target :size="24" />
            <div class="mode-info">
              <span class="mode-name">Bullseye</span
              ><span class="mode-desc">Année Exacte</span>
            </div>
            <ChevronRight :size="20" style="margin-left: auto; opacity: 0.5" />
          </button>
          <button
            class="btn-primary btn-mode btn-memory"
            @click="setGameMode('memory')"
          >
            <Brain :size="24" />
            <div class="mode-info">
              <span class="mode-name">Memory</span
              ><span class="mode-desc">Paires Audio</span>
            </div>
            <ChevronRight :size="20" style="margin-left: auto; opacity: 0.5" />
          </button>
        </div>
        ...
        <Transition name="fade" mode="out-in">
          ...
          >
            <Transition name="fade" mode="out-in">
              ...
              <TimelinePlayer
                v-else-if="
                  currentView === 'timeline-player' && timelineMysterySong
                "
              ...
              <MemoryGame v-else-if="currentView === 'memory-player'" />
            </Transition>
          </div>
        </Transition>
      </template>
    ...
    <div
      class="dynamic-bg"
      :class="{ active: isBgAActive, 'has-cover': !!currentCoverA }"
      :style="currentCoverA ? { '--bg-img': `url(${currentCoverA})` } : {}"
    ></div>
    <div
      class="dynamic-bg"
      :class="{ active: !isBgAActive, 'has-cover': !!currentCoverB }"
      :style="currentCoverB ? { '--bg-img': `url(${currentCoverB})` } : {}"
    ></div>
  </div>
</template>
...
.logo h1 {
  ...
  line-height: 1.2;
  margin: 0;
  background: linear-gradient(to bottom right, #ffffff 20%, #fbbf24 100%);
...
.dynamic-bg {
  ...
  z-index: -1;
  filter: blur(60px) brightness(0.9) saturate(1.8);
  opacity: 0;
  transition: opacity 2s ease-in-out;
  pointer-events: none;
}
.dynamic-bg.active.has-cover {
  opacity: 1;
}
</style>
