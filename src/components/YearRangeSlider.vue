...
// Default minGap manually if needed, or use withDefaults. 
// Standard TS defineProps doesn't set defaults without withDefaults.
// But we can just use (props.minGap || 1) in code.
...
const minGap = computed(() => props.minGap ?? 1);

const minVal = computed({
  get: () => props.modelValue[0] || props.min,
  set: (val: number) => {
    // Logical collision only (1 year gap)
    const limit = (props.modelValue[1] || props.max) - minGap.value;
    const newVal = Math.min(val, limit);
    emit("update:modelValue", [newVal, props.modelValue[1] || props.max]);
    updateDragState(newVal);
  },
});

const maxVal = computed({
  get: () => props.modelValue[1] || props.max,
  set: (val: number) => {
    const limit = (props.modelValue[0] || props.min) + minGap.value;
    const newVal = Math.max(val, limit);
    emit("update:modelValue", [props.modelValue[0] || props.min, newVal]);
    updateDragState(newVal);
  },
});

const updateDragState = (val: number) => {
...
const leftPercentNum = computed(
  () => ((props.modelValue[0] - props.min) / (props.max - props.min)) * 100,
);
const rightPercentNum = computed(
...
const leftInputStyle = computed(() => ({
  transform: isColliding.value ? "translateY(-24px)" : "translateY(0)",
  zIndex: isColliding.value ? 20 : 10,
}));
...
<template>
  <div class="slider-container">
    <div class="slider-wrapper">
      ...
      <div
        class="track-fill"
        :style="{ left: leftPercent, right: `calc(100% - ${rightPercent})` }"
      ></div>
...
.handle {
  ...
  transition:
    ...
    left 0.1s linear; /* Bouncy vertical move, linear horizontal */
}
